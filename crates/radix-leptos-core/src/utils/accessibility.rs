...
pub fn get_or_create_live_region(politeness: AriaLive) -> Option<HtmlElement> {
    ...
    live_region.set_attribute("aria-live", politeness.as_str()).ok()?;
    live_region.set_attribute("aria-atomic", "true").ok()?;
    live_region.set_attribute("style", "position: absolute; left: -10000px; width: 1px; height: 1px; overflow: hidden;").ok()?;
...
mod tests {
    ...
    fn test_aria_attributes() {
        ...
        assert_eq!(element.get_attribute("aria-label").as_deref(), Some("Close dialog"));
        assert_eq!(element.get_attribute("aria-expanded").as_deref(), Some("true"));
        assert_eq!(element.get_attribute("aria-disabled").as_deref(), Some("false"));
    }
    ...
    fn test_live_region_creation() {
        ...
        assert_eq!(live_region.get_attribute("aria-live").as_deref(), Some("polite"));
        assert_eq!(live_region.get_attribute("aria-atomic").as_deref(), Some("true"));
    }
}