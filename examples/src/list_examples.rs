...
pub fn ListExamples() -> impl IntoView {
    ...
    let (large_dataset_selected_items, set_large_dataset_selected_items) =
    ...
    let handle_multi_selection = Callback::new(move |selected: Vec<String>| {
        ...
        web_sys::console::log_1(
            &format!("Multi-select list selection: {:?}", selected_clone).into(),
        );
    });
    ...
    let handle_virtualized_selection = Callback::new(move |selected: Vec<String>| {
        ...
        web_sys::console::log_1(
            &format!("Virtualized list selection: {:?}", selected_clone).into(),
        );
    });
