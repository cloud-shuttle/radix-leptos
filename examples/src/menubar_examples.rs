...
pub fn MenubarExamples() -> impl IntoView {
    ...
    let (zoom_50_checked, set_zoom_50_checked) = signal(zoom_level.get() == "50%");
    let (zoom_100_checked, set_zoom_100_checked) = signal(zoom_level.get() == "100%");
    let (zoom_150_checked, set_zoom_150_checked) = signal(zoom_level.get() == "150%");
    let (theme_light_checked, set_theme_light_checked) = signal(selected_theme.get() == "light");
    let (theme_dark_checked, set_theme_dark_checked) = signal(selected_theme.get() == "dark");
    let (theme_auto_checked, set_theme_auto_checked) = signal(selected_theme.get() == "auto");

    // Update computed signals when dependencies change
    Effect::new(move |_| {
        set_zoom_50_checked.set(zoom_level.get() == "50%");
    });
    Effect::new(move |_| {
        set_zoom_100_checked.set(zoom_level.get() == "100%");
    });
    Effect::new(move |_| {
        set_zoom_150_checked.set(zoom_level.get() == "150%");
    });
    Effect::new(move |_| {
        set_theme_light_checked.set(selected_theme.get() == "light");
    });
    Effect::new(move |_| {
        set_theme_dark_checked.set(selected_theme.get() == "dark");
    });
    Effect::new(move |_| {
        set_theme_auto_checked.set(selected_theme.get() == "auto");
    });
    ...
    let handle_theme_change = Callback::new(move |theme: String| {
        set_selected_theme.set(theme.clone());
